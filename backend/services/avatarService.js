...
const proxyGoogleAvatar = async (userId) => {
  try {
    ...
    console.error('Avatar proxy service error:', error);
    ...
    if (error.code === 'ECONNABORTED') {
      return {
        success: false,
        status: 408,
        error: 'Request timeout while fetching avatar'
      };
    }
    ...
    if (error.response) {
      return {
        ...
        error: 'Failed to fetch avatar from Google'
      };
    }
    ...
    return {
      success: false,
      status: 500,
      error: 'Internal server error while processing avatar'
    };
  }
};
...
const getUserAvatarUrl = async (userId) => {
  try {
    ...
    console.error('Get avatar URL service error:', error);
    return {
      success: false,
      status: 500,
      error: 'Failed to fetch user avatar information'
    };
  }
};
